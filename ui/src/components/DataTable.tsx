...
const DataTable: FC<Props> = ({ headers, rows}) => {
  ...
  const headersMap = useRef(Object.fromEntries(headers.map((item, index) => [item, index])));
  ...
  const isSortable = (column: string) => {
    ...
    if (column !== '' && rows.length > 0 && rows[0][itemIndex].sortKey !== undefined) {
      return true;
    }
    return false;
  }
  ...
  if (isSortable(sortProps.column)) {
    rows.sort((a, b) => {
      ...
      if (sortProps.order === 'asc') {
        ...
        } else if (typeof aSortKey === 'string' && typeof bSortKey == 'string') {
      ...
      return (
        <th
          ...
          onClick={sortEnabled ? () => handleHeaderClick(item) : undefined }>
            { item }
          { item === sortProps.column && sortProps.order == 'asc' && (<MdArrowDropUp size={20} />)}
          { item === sortProps.column && sortProps.order == 'desc' && (<MdArrowDropDown size={20} />)}
        </th>
      );
    });
