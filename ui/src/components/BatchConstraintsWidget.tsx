...
const BatchConstraintsWidget: FC<Props> = ({ value, onChange }) => {
  ...
  const [constraint, setConstraint] = useState<BatchConstraint>({...INITIAL_CONSTRAINT});
  ...
  function updateConstraintField<T, K extends keyof T>(obj: T, key: K, value: T[K]) {
    obj[key] = value;
  }
            ...
            <tr key={index}>
              <td style={{ display: 'flex', gap: '8px' }}>
                ...
                <Form.Control
                  ...
                  onChange={(e) => handleEdit(index, "include_expression", e.target.value)}
                />
                <Form.Control
                  type="number"
                  size="sm"
                  value={item.max_concurrent_instances}
                  onChange={(e) => handleEdit(index, "max_concurrent_instances", e.target.value)}
                />
                <Form.Control
                  type="text"
                  size="sm"
                  value={item.min_instance_boot_time}
                  onChange={(e) => handleEdit(index, "min_instance_boot_time", e.target.value)}
                />
              </td>
              <td>
          ...
          <tr>
            <td style={{ display: 'flex', gap: '8px' }}>
              ...
              <Form.Control
                ...
                onChange={(e) => setConstraint({...constraint, include_expression: e.target.value})}
              />
              <Form.Control
                type="number"
                size="sm"
                ...
                onChange={(e) => setConstraint({...constraint, max_concurrent_instances: Number(e.target.value)})}
              />
              <Form.Control
                type="text"
                size="sm"
                ...
                onChange={(e) => setConstraint({...constraint, min_instance_boot_time: e.target.value})}
              />
            </td>
            <td>
