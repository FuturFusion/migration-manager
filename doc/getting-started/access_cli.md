...
This certificate should be added to the `trusted_tls_client_cert_fingerprints` list with the local CLI tool using `migration-manager system security edit` for the remote CLI to properly function. Alternatively, add the full X509 PEM encoded certificate to `trusted_tls_client_certificates`; Migration Manager derives and trusts its SHA256 fingerprint.
