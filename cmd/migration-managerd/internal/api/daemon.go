...
func (d *Daemon) Start() error {
	...
	d.server.Addr = net.JoinHostPort(d.config.Network.Address, strconv.Itoa(d.config.Network.Port))
	...
	errChan := d.updateHTTPListener(d.server.Addr)
	err = <-errChan
	if err != nil {
		return err
	}
...
func (d *Daemon) updateServerCert(cfg api.Certificate) (_err error) {
	d.configLock.Lock()
	defer d.configLock.Unlock()
