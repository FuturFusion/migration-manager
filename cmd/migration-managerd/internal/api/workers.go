...
func (d *Daemon) processQueuedBatches(ctx context.Context) error {
	...
	var sourcesByInstance map[uuid.UUID]migration.Source
	err = transaction.Do(ctx, func(ctx context.Context) error {
...
func (d *Daemon) initMigrationWorkers(ctx context.Context, cleanupInstances bool) error {
	...
	var targetsByBatch map[int]migration.Target
	var sourcesByInstance map[uuid.UUID]migration.Source
	err := transaction.Do(ctx, func(ctx context.Context) error {
...
func (d *Daemon) finalizeCompleteInstances(ctx context.Context) error {
	...
	var targetsByInstance map[uuid.UUID]target.InternalIncusTarget
	...
	err = transaction.Do(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			return fmt.Errorf("Failed to get all networks")
		}

		targetsByInstance = make(map[uuid.UUID]target.InternalIncusTarget, len(instances))
		...
		for _, i := range instances {
			...
			if err != nil {
				return fmt.Errorf("Failed to get target for instance %q: %w", i.UUID, err)
			}

			// Get the batch for this instance.
			batch, err := d.batch.GetByID(ctx, *i.BatchID)
			if err != nil {
				return fmt.Errorf("Failed to get batch for instance %q: %w", i.UUID, err)
			}
