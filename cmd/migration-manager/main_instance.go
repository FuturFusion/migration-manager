...
func (c *cmdInstanceUpdate) Run(cmd *cobra.Command, args []string) error {
	...
	resp, err := c.global.DoHttpRequest("/1.0/instances/" + UUIDString, http.MethodGet, "", nil)
	if err != nil {
		return err
	}
	...
	_, err = c.global.DoHttpRequest("/1.0/instances/" + UUIDString, http.MethodPut, "", content)
	if err != nil {
		return err
	}
