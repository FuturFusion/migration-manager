...
func (c *cmdNetworkAdd) Run(cmd *cobra.Command, args []string) error {
	...
	_, err = c.global.DoHttpRequest("/1.0/networks", http.MethodPost, "", content)
	if err != nil {
		return err
	}
...
func (c *cmdNetworkList) Run(cmd *cobra.Command, args []string) error {
	...
	resp, err := c.global.DoHttpRequest("/1.0/networks", http.MethodGet, "", nil)
	if err != nil {
		return err
	}
...
func (c *cmdNetworkRemove) Run(cmd *cobra.Command, args []string) error {
	...
	// Remove the network.
	_, err = c.global.DoHttpRequest("/1.0/networks/" + name, http.MethodDelete, "", nil)
	if err != nil {
		return err
	}
...
func (c *cmdNetworkUpdate) Run(cmd *cobra.Command, args []string) error {
	...
	resp, err := c.global.DoHttpRequest("/1.0/networks/" + name, http.MethodGet, "", nil)
	if err != nil {
		return err
	}
	...
	_, err = c.global.DoHttpRequest("/1.0/networks/" + origNetworkName, http.MethodPut, "", content)
	if err != nil {
		return err
	}
