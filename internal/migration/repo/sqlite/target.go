...
func (t target) Create(ctx context.Context, in migration.Target) (migration.Target, error) {
	const sqlUpsert = `
INSERT INTO targets (name, endpoint, tls_client_key, tls_client_cert, oidc_tokens, insecure, incus_project)
VALUES(:name, :endpoint, :tls_client_key, :tls_client_cert, :oidc_tokens, :insecure, :incus_project)
RETURNING id, name, endpoint, tls_client_key, tls_client_cert, oidc_tokens, insecure, incus_project;
`
	...
	row := t.db.QueryRowContext(ctx, sqlUpsert,
		...
		sql.Named("incus_project", in.IncusProject),
	)
	if row.Err() != nil {
...
func (t target) GetAll(ctx context.Context) (migration.Targets, error) {
	const sqlGetAll = `SELECT id, name, endpoint, tls_client_key, tls_client_cert, oidc_tokens, insecure, incus_project FROM targets ORDER BY name`
...
func (t target) GetByID(ctx context.Context, id int) (migration.Target, error) {
	const sqlGetByName = `SELECT id, name, endpoint, tls_client_key, tls_client_cert, oidc_tokens, insecure, incus_project FROM targets WHERE id=:id`
...
func (t target) GetByName(ctx context.Context, name string) (migration.Target, error) {
	const sqlGetByName = `SELECT id, name, endpoint, tls_client_key, tls_client_cert, oidc_tokens, insecure, incus_project FROM targets WHERE name=:name`
...
func (t target) UpdateByName(ctx context.Context, in migration.Target) (migration.Target, error) {
	const sqlUpsert = `
UPDATE targets SET name=:name, endpoint=:endpoint, tls_client_key=:tls_client_key, tls_client_cert=:tls_client_cert, oidc_tokens=:oidc_tokens, insecure=:insecure, incus_project=:incus_project
WHERE name=:name
RETURNING id, name, endpoint, tls_client_key, tls_client_cert, oidc_tokens, insecure, incus_project;
`
...
func scanTarget(row interface{ Scan(dest ...any) error }) (migration.Target, error) {
	...
	err := row.Scan(&target.ID, &target.Name, &target.Endpoint, &target.TLSClientKey, &target.TLSClientCert, &marshalledOIDCTokens, &target.Insecure, &target.IncusProject)
	if err != nil {
