...
SELECT batches.id, batches.name, targets.name AS target, batches.target_project, batches.status, batches.status_string, batches.storage_pool, batches.include_expression, batches.migration_window_start, batches.migration_window_end
  FROM batches
...
UPDATE batches
  SET name = ?, target_id = (SELECT targets.id FROM targets WHERE targets.name = ?), target_project = ?, status = ?, status_string = ?, storage_pool = ?, include_expression = ?, migration_window_start = ?, migration_window_end = ?
 WHERE id = ?
`)
...
func batchColumns() string {
	return "batches.id, batches.name, targets.name AS target, batches.target_project, batches.status, batches.status_string, batches.storage_pool, batches.include_expression, batches.migration_window_start, batches.migration_window_end"
}
...
func getBatches(ctx context.Context, stmt *sql.Stmt, args ...any) ([]migration.Batch, error) {
	...
	dest := func(scan func(dest ...any) error) error {
		b := migration.Batch{}
		err := scan(&b.ID, &b.Name, &b.Target, &b.TargetProject, &b.Status, &b.StatusString, &b.StoragePool, &b.IncludeExpression, &b.MigrationWindowStart, &b.MigrationWindowEnd)
		if err != nil {
			return err
		}
...
func UpdateBatch(ctx context.Context, db tx, name string, object migration.Batch) (_err error) {
	...
	result, err := stmt.Exec(object.Name, object.Target, object.TargetProject, object.Status, object.StatusString, object.StoragePool, object.IncludeExpression, object.MigrationWindowStart, object.MigrationWindowEnd, id)
	if err != nil {
