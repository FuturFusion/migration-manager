	...
	for _, instanceUUID := range instances {
		...
		if len(batches) > 0 {
			for _, b := range batches {
				if b.Status != api.BATCHSTATUS_DEFINED && b.Status != api.BATCHSTATUS_FINISHED {
					return nil, fmt.Errorf("Instance %q cannot be modified because it is part of an active batch", instanceUUID)
				}
			}
		}
	}
