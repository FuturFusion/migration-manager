...
func (o *OIDCClient) Do(req *http.Request) (*http.Response, error) {
	...
	if o.oidcContext.TrustTuple.Issuer != resp.Header.Get("X-MigrationManager-OIDC-issuer") && resp.Header.Get("X-MigrationManager-OIDC-issuer") != "" {
		o.oidcContext.TrustTuple.Issuer = resp.Header.Get("X-MigrationManager-OIDC-issuer")
		trustTupleChanged = true
	}

	if o.oidcContext.TrustTuple.ClientID != resp.Header.Get("X-MigrationManager-OIDC-clientid") && resp.Header.Get("X-MigrationManager-OIDC-clientid") != "" {
		o.oidcContext.TrustTuple.ClientID = resp.Header.Get("X-MigrationManager-OIDC-clientid")
		trustTupleChanged = true
	}

	if o.oidcContext.TrustTuple.Audience != resp.Header.Get("X-MigrationManager-OIDC-audience") && resp.Header.Get("X-MigrationManager-OIDC-audience") != "" {
		o.oidcContext.TrustTuple.Audience = resp.Header.Get("X-MigrationManager-OIDC-audience")
		trustTupleChanged = true
	}
...
func (o *OIDCClient) refresh(issuer string, clientID string) error {
	if o.oidcContext.Tokens.Token == nil || o.oidcContext.Tokens.RefreshToken == "" {
